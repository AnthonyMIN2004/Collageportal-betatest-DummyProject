# ── COLLEGIATE PORTAL | SEED_DATA.PY ──
# Curriculum constants, ported from the frontend js/data.js so the
# backend is the single source of truth. Used by database.py to seed
# the SQLite tables and by the schedule/classes routes.

# Class period times (used by the frontend timetable)
PERIODS = [
...
# Exams — seeded into the `exams` table
EXAMS = [
...
# Campus events — seeded into the `events` table (date stored ISO)
EVENTS = [
    {"name": "オープンキャンパス", "date": "2026-06-20", "detail": "メインホール 10:00〜"},
    {"name": "前期試験開始", "date": "2026-07-10", "detail": "時間割を確認すること"},
    {"name": "夏休み開始", "date": "2026-07-25", "detail": "良い夏を！🎉"},
]

# Per-class syllabus info
CLASS_INFO = {
    "DX社会学":              {"icon": "🌐", "desc": "デジタル社会と社会学の交差点を探求します。"},
    "教育テック実学":         {"icon": "🎓", "desc": "教育テクノロジーの実践的な活用方法を学びます。"},
    "保育原理":               {"icon": "👶", "desc": "保育の基本原理と子どもの発達を学びます。"},
    "地域協働実践":           {"icon": "🤝", "desc": "地域社会との協働プロジェクトを実践します。"},
    "ゼミ":                   {"icon": "💡", "desc": "自由研究・プロジェクト発表のゼミです。"},
    "クラウドとビッグデータ": {"icon": "☁️", "desc": "クラウドインフラとデータ処理の基礎を学びます。"},
    "聖書と現代人":           {"icon": "📖", "desc": "聖書の教えと現代社会との関係を考察します。"},
    "スポーツ実技":           {"icon": "⚽", "desc": "体育館でのスポーツ実技。動きやすい服装で来ること！"},
    "WebUIデザイン html/css": {"icon": "🎨", "desc": "HTML・CSSを使ったWebデザインの基礎を学びます。"},
    "WebスクリプトJS":        {"icon": "⚡", "desc": "JavaScriptによるインタラクティブなWeb開発。"},
    "並列計算":               {"icon": "🔢", "desc": "並列処理・マルチスレッドプログラミングを学びます。"},
    "生成AIによるpython":     {"icon": "🤖", "desc": "生成AIとPythonプログラミングを組み合わせた実践。"},
}

# Items each class needs the student to bring
CLASS_BRING_ITEMS = {
...
# Unique class names, order preserved
ALL_CLASSES = list(dict.fromkeys(c["name"] for day in SCHEDULE.values() for c in day))

# Seeded students. ID range 25501–25510 matches the frontend gate.
# Default password == the student ID (CHANGE THESE before going live).
STUDENT_IDS = [str(25501 + i) for i in range(10)]

# Default tasks given to a fresh student account
DEFAULT_TASKS = [
    "Finish DX社会学 homework",
    "Review Python API connection codes",
    "Check campus open day guide",
]
